...
\begin{proof}
	Как и в теореме выше, будем считать, что $z_n = \infty$. Выберем $R > 0$ такое, что $z_1, \dotsc, z_n \in B_R(0)$, и применим теорему к областям $B_R(0)$ и $\overline{B_R(0)}$:
	\[\int_{|z| = R}fdz = 2\pi i\sum_{k = 1}^{n-1}\res_{z_k}f = -2\pi i\res_{\infty}f = -2\pi i\res_{z_n}f \ra \sum_{k = 1}^n\res_{z_k}f = 0\]
