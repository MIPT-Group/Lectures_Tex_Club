	...
	\begin{center}
		\scalebox{1.1}{
			\begin{tikzpicture}
				...
				\fill [opacity=0.03] (0,0) (-0.8, -0.8) circle[radius=2 - 0.8 * 1.41421];
				...
				\node[] at (-0.25, 0.3) {\scalebox{0.85}{$f \equiv 0$}};
			\end{tikzpicture}
		}
	\end{center}
	
	Положим $\tau := \inf\{t \in [a, b]: f(\sigma(t)) \ne 0\}$, $z^* = \sigma(\tau)$, тогда $\tau \in (a, b)$ и, в силу непрерывности, $f(z^*) = 0$, причем нуль $z^*$ "--- не изолированный. Но тогда $f \equiv 0$ на некотором круге $K^*$ с центром в точке $z^*$, что противоречит выбору $\tau$.
\end{proof}
