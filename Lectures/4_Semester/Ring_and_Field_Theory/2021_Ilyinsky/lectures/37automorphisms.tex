...
\begin{note}
	Легко проверить, что $L^H$ действительно является полем. Однако в общем случае неверно, что сопоставление $K \mapsto \Aut_{K}L$ осуществляет биекцию между полями $K$ такими, что $F \subset K \subset L$, и подгруппами в $\Aut_L{F}$. Далее мы будем исследовать достаточные условия того, чтобы это было выполнено.
\end{note}
